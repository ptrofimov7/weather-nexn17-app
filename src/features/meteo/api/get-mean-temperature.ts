...
export const getHistoricalMeanTemperature = async (
  ids?: Array<number>
): Promise<HistoricalMeanTempareture[]> => {
    ...
    citiesData.map(
      ({ latitude, longitude }: { latitude: number; longitude: number }) => {
        return apiForecastClient.get(
          `/forecast?latitude=${latitude}&longitude=${longitude}&daily=temperature_2m_max,temperature_2m_min&past_days=7&forecast_days=0&timezone=auto`
        ) as Promise<any>;
      }
    )
  ...
  data.forEach((element) => {
    ...
    times.forEach((_: unknown, index: number) => {
      const temperature = roundValue(
        (temperature_2m_min[index] + temperature_2m_max[index]) / 2
      );
  ...
  const result = Object.entries(temp).map((element) => {
    ...
    const temperature_mean = roundValue(
      temperature_mean_values.reduce((acc: number, cur: number | null) => {
        return acc + (cur ?? 0);
...
export const useHistoricalMeanTemperature = (ids?: Array<number> | number) => {
  const { data, isFetching, isFetched } = useQuery({
    ...
    queryFn: () =>
      getHistoricalMeanTemperature(typeof ids === "number" ? [ids] : ids),
    initialData: [],
  });
